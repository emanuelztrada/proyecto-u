...
const AUTENTICAR_USUARIO = gql`
    mutation authenticationUser($input: AuthenticationUser){
...
const Login = () => {
    ...
    const [ authenticationUser ] = useMutation(AUTENTICAR_USUARIO); 
    ...
    const formik = useFormik({
        ...
        onSubmit: async  valores =>{
            ...
            try {
                ...
                setTimeout(() =>{
                    const {token} = data.authenticationUser; 
                    localStorage.setItem('token', token); 
                }, 10); 

                //Redireccionar a cleintes 
                setTimeout(async () => {
                    guardarMensaje(null);
                    router.push('/');
                }, 2000); 
